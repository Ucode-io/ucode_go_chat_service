...
import (
	...
	"ucode/ucode_go_chat_service/internal/socketio/engineio"
	"ucode/ucode_go_chat_service/internal/socketio/socket_protocol"
...
func (s *Socket) Emit(event string, agrs ...any) error {
	c := s.Conn
	if c == nil {
		return errors.New("socket has disconnected")
	}
	agrs = append([]any{event}, agrs...)
	return s.writer(socket_protocol.EVENT, agrs)
}

func (s *Socket) ack(ackEvent string, agrs ...any) error {
	c := s.Conn
	if c == nil {
		return errors.New("socket has disconnected")
	}
	agrs = append([]any{ackEvent}, agrs...)
...
func (s *Socket) engineWrite(t engineio.PacketType, arg ...any) error {
...
func (s *Socket) writer(t socket_protocol.PacketType, arg ...any) error {
	w, err := s.Conn.NextWriter(websocket.TextMessage)
	if err != nil {
		return err
	}
	...
	if t == socket_protocol.ACK {
		agrs := append([]any{}, arg[0].([]any)[1:])
		socket_protocol.WriteToWithAck(w, t, nps, arg[0].([]any)[0].(string), agrs...)
	} else {
