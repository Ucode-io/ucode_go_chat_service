...
func (s *Io) Emit(event string, agrs ...any) error {
			...
			if len(dataJson) > 0 {
				if reflect.TypeOf(dataJson[0]).String() == "string" {
					...
					for _, callback := range payLoad.socket.listeners.get(event) {
						data := append([]any{}, dataJson[1:]...)

						ackCallback := AckCallback(func(data ...any) {
					...
					case socket_protocol.CONNECT.String():
						...
						if namespace != "/" {
							...
							if nps := s.namespaces.get(namespace); nps == nil {
								socket_nps.writer(socket_protocol.CONNECT_ERROR, map[string]any{
