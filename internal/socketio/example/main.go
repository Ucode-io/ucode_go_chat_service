...
import (
	...
	socketio "ucode/ucode_go_chat_service/internal/socketio"
...
func socketIoHandle(io *socketio.Io) {
	...
	io.OnConnection(func(socket *socketio.Socket) {
		...
		socket.On("chat message", func(event *socketio.EventPayload) {
			...
			if len(event.Data) > 2 {
				log.Println(socket.Nps, ": ", event.Data[2].(map[string]any))
			}

			if event.Ack != nil {
				event.Ack("hello from name space root", map[string]any{
					"Test": "ok",
				})
			}
		})
	...
	io.Of("/test").OnConnection(func(socket *socketio.Socket) {
		...
		socket.On("chat message", func(event *socketio.EventPayload) {
			...
			if event.Ack != nil {
				event.Ack("hello from nps test", map[string]any{
					"Test": "ok",
				})
			}
		})
